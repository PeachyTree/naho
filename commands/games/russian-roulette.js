	...
	async run(msg) {
		...
		if (balance < money) {
			return msg.reply(stripIndents`
				...
				You need ${Currency.convert(money)} to join.
			`);
		}
		...
		if (roulette) {
			...
			roulette.join(msg.author, money);
		...
		`).then(async () => {
			...
			Currency.removeBalance(deadPlayer.user.id, money);
			survivors.forEach(survivor => Currency.addBalance(survivor.user.id, money / survivors.length));
			...
			return msg.embed({
				description: stripIndents`
					...
					__**Reward**__
					The survivors receive ${Currency.convert(money / survivors.length)} each.
				`
			});
		});
	}
