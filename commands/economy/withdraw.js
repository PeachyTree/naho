...
module.exports = class WidthdrawCommand extends Command {
	constructor(client) {
		super(client, {
			...
			args: [
				{
					key: 'money',
					label: 'amount of money to withdraw',
	...
	async run(msg, { money }) {
		if (money <= 0) return msg.reply(`you can't widthdraw 0 or less ${Currency.convert(0)}.`);
		...
		Bank.withdraw(msg.author.id, money);

		return msg.reply(`successfully withdrew ${Currency.convert(money)} from the bank!`);
	}
};
