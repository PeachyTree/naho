...
module.exports = class BuyItemCommand extends Command {
	constructor(client) {
		super(client, {
			...
			details: 'Let\'s you exchange your hard earned money for other goods.',
			throttling: {
				usages: 2,
				duration: 3
			},
	...
	async run(msg, { amount, item }) {
		...
		if (balance < storeItem.price * amount) {
			...
			return msg.reply(stripIndents`
				you don't have enough money to buy ${amount} ${itemName}${plural ? 's' : ''}. ${amount} ${itemName}${plural ? 's' : ''} cost${plural ? '' : 's'} ${amount * storeItem.price} 🍩s.
